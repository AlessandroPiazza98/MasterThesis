...
parser.add_argument('-to', "--token_overlap", type=int, default=0) #Hidden layers if requested
...
if __name__ == "__main__":
    ...
    encoder = GraphEncoder(input_dim, hidden, edge_dim, output_dim, dropout).to(device)
    decoder = GraphDecoder(output_dim, hidden, input_dim, edge_dim, dropout).to(device)
        ...
        def train():
            ...
            for data in train_loader:  #Iterate in batches over the training dataset.
                ...
                for graph in data.to_data_list():
                    tokens = []
                    windowed_data = windowing(graph, ntoken, device, token_overlap) #TODO
                    for win_graph in windowed_data:
                        representation = model(win_graph)
